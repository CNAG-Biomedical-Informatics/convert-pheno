...
sub _cdisc2redcap_longitudinal {
    ...
    my $subjects    = $data->{ODM}{ClinicalData}{SubjectData};    # uncoverable statement
                                                                  # uncoverable statement
    ...
    for my $subject ( @{$subjects} ) {                            # uncoverable statement
        ...
        for my $StudyEventData ( @{ $subject->{'StudyEventData'} } ) {    # uncoverable statement
            my $UniqueEventName =
              $StudyEventData->{'-redcap:UniqueEventName'};               # uncoverable statement
                                                                          # uncoverable statement
                                                                          # FormData # uncoverable statement
            for my $FormData ( @{ $StudyEventData->{FormData} } ) {       # uncoverable statement
                                                                          # uncoverable statement
                                                                          # ItemGroupData # uncoverable statement
                for my $ItemGroupData ( @{ $FormData->{ItemGroupData} } ) {    # uncoverable statement
                                                                               # uncoverable statement
                                                                               # The elements can arrive as {} or [] # uncoverable statement
                                                                               # Both will be loaded as [] # uncoverable statement
                    if ( ref $ItemGroupData->{ItemData} eq ref [] ) {          # uncoverable statement
                        for my $ItemData ( @{ $ItemGroupData->{ItemData} } ) { # uncoverable statement
                            push                                               # uncoverable statement
                              @{
                                $individual->{$key}{ $ItemData->{'-ItemOID'} } # uncoverable statement
                              },
                              { $UniqueEventName => $ItemData->{'-Value'} };   # uncoverable statement
                        }    # uncoverable statement
                    }    # uncoverable statement
                        ...
                        $individual->{$key}                                 # uncoverable statement
                          { $ItemGroupData->{ItemData}{'-ItemOID'} } = [    # uncoverable statement
                            {                                               # uncoverable statement
                                $UniqueEventName =>                         # uncoverable statement
                                  $ItemGroupData->{ItemData}{'-Value'}      # uncoverable statement
                            }    # uncoverable statement
                          ];    # uncoverable statement
                    }    # uncoverable statement
                }    # uncoverable statement
