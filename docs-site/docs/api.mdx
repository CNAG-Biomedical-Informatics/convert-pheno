...
The Mojolicious API accepts self-contained JSON and registry-defined multipart
uploads. Use the [command-line interface](use-as-a-command-line-interface) for
...
Read route options and current resource availability from the catalog instead
of maintaining a separate conversion list in the client.

`maturity` is optional and describes the conversion's source profile, not the
HTTP interface. The desktop application shows an Experimental badge only when the catalog
explicitly supplies that designation.
