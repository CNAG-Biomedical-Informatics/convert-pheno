...
<figcaption>Supported conversions. Dotted blue lines go via BFF, which serves as the internal target model for most routes.</figcaption>
...
`BFF` is the internal target model for most conversions. User-facing output, however, is not limited to `BFF`: Convert-Pheno also supports `PXF` and `OMOP CDM` output paths.
:::
