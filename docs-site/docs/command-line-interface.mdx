...
`Convert-Pheno` includes a **command-line utility** for file-based conversions. This is the **primary way** most users work with the project. Choose the input format first, then select a supported output and copy the corresponding command.
...
<div className="button-row">
  <Link className="button button--primary" to="/conversion-recipes">Commands by input format</Link>
...
- The route is also available through the Mojolicious HTTP(s) API and desktop application.
...
- [CLI conversion commands](conversion-recipes) grouped by input format
