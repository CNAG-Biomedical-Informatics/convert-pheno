...
- `-ipxf`, `-ibff`, `-iomop`, `-iopenehr`, `-iredcap`, `-icdisc-odm`, `-idataset-json`, `-icsv`
- `-obff`, `-opxf`, `-oomop`, `-ocsv`, `-ojsonf`, `-ojsonld`

:::note[Experimental source profiles]
`openEHR` canonical input and CDISC Dataset-JSON SDTM input are implemented but
marked experimental while independent source coverage grows. Dataset-JSON can
write BFF, PXF, or OMOP-CDM; openEHR currently writes BFF or PXF.
...
- `biosamples` are emitted from PXF biosample data, OMOP `SPECIMEN`, or explicit `beacon.biosamples.rules` in CSV, REDCap, and CDISC-ODM mappings.
...
- `--no-source-info` omits raw source provenance copied into `BFF` `info`, such as `OMOP_columns`, `CSV_columns`, `REDCap_columns`, and `datasetJson.domains`. Mapped fields and `info.convertPheno` are kept.
