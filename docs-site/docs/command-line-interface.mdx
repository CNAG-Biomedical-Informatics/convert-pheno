...
- `-ipxf`, `-ibff`, `-icbioportal`, `-iomop`, `-iopenehr`, `-iredcap`, `-icdisc-odm`, `-idataset-json`, `-ifhir`, `-icsv`
- `-obff`, `-opxf`, `-oomop`, `-ocsv`, `-ojsonf`, `-ojsonld`

:::note[Experimental source profiles]
`cBioPortal` clinical packages, openEHR canonical input, CDISC Dataset-JSON SDTM input, and FHIR R4 Bundle
...
- `--no-source-info` omits raw source provenance copied into `BFF` `info`, such as `OMOP_columns`, `CSV_columns`, `REDCap_columns`, `CDISC_ODM`, `cbioportal`, `datasetJson.domains`, and `fhir` resources. Mapped fields and `info.convertPheno` are kept.
...
- `--mapping-file FILE` supplies a Mapping V2 YAML or JSON document targeting Beacon schema `2.0.0`, required by CSV, REDCap, and CDISC-ODM routes and optional for cBioPortal clinical packages.
