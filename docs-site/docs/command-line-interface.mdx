...
Dataset-JSON and FHIR can write BFF, PXF, or OMOP-CDM; openEHR currently writes
BFF or PXF.
...
- `--no-source-info` omits raw source provenance copied into `BFF` `info`, such as `OMOP_columns`, `CSV_columns`, `REDCap_columns`, `CDISC_ODM`, `datasetJson.domains`, and `fhir` resources. Mapped fields and `info.convertPheno` are kept.
