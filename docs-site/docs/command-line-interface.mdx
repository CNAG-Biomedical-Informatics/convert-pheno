...
- `--term-audit-tsv FILE` writes a tab-separated audit of direct terms, identifier lookups, label searches, and fallbacks, including the effective search mode and provenance of each decision.
...
- `--term-audit-tsv FILE` writes a TSV report of terminology decisions. The audit includes source values and labels, actual lookup inputs, converted terms, provenance, fallbacks, and effective search settings.
...
For resolution precedence, audit columns, and threshold tradeoffs, see [Terminology Search](tbl/db-search).
