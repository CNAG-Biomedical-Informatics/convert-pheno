...
- `-ipxf`, `-ibff`, `-iomop`, `-iopenehr`, `-iredcap`, `-icdisc-odm`, `-idataset-json`, `-ifhir`, `-icsv`
- `-obff`, `-opxf`, `-oomop`, `-ocsv`, `-ojsonf`, `-ojsonld`

:::note[Experimental source profiles]
`openEHR` canonical input, CDISC Dataset-JSON SDTM input, and FHIR R4 Bundle
input are implemented but marked experimental while independent source coverage
grows. Dataset-JSON and FHIR can write BFF, PXF, or OMOP-CDM; openEHR currently
writes BFF or PXF.
...
- `biosamples` are emitted from PXF biosample data, FHIR `Specimen`, OMOP `SPECIMEN`, or explicit `beacon.biosamples.rules` in CSV, REDCap, and CDISC-ODM mappings.
...
- These mapping features are available only for `csv2bff`, `redcap2bff`, and `cdiscodm2bff`, which are the routes that read a mapping file.
...
- `--no-source-info` omits raw source provenance copied into `BFF` `info`, such as `OMOP_columns`, `CSV_columns`, `REDCap_columns`, `datasetJson.domains`, and `fhir` resources. Mapped fields and `info.convertPheno` are kept.
- `--search-audit-tsv FILE` writes a tab-separated audit of ontology search results for mapping-file-driven conversions such as `csv2bff`, `redcap2bff`, and `cdiscodm2bff`, including the effective configured search mode, whether each lookup matched the DB or fell back to `NA`, and the per-row lookup resolution (`exact`, `similarity`, or `fallback_na`).
...
- `--mapping-file FILE` supplies a Mapping V2 YAML or JSON document targeting Beacon schema `2.0.0`, used by `csv2bff`, `redcap2bff`, `cdiscodm2bff`, and related conversions.
- `--redcap-dictionary FILE` or `-rcd FILE` supplies the REDCap-compatible data dictionary required by REDCap and CDISC-ODM input conversions.
...
- Exactly one `DM` dataset is required. Subject-level domains are grouped by `USUBJID`; a mapping file and REDCap dictionary are not used.
