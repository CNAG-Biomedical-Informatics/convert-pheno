...
For more examples, see [Choose a Conversion](conversion-recipes).
...
<OmopCsvExampleDownloads /> Keep the three files together and pass all of them after `-iomop`.
...
Most examples below use the individuals-only `-obff FILE` path, which still emits Beacon `individuals` as one file. In non-stream BFF conversions, you can also request synthesized `datasets` and `cohorts` with `--entities ... --out-dir out/`. `biosamples` can now be emitted from OMOP `SPECIMEN`; this path is implemented and regression-tested, while broader validation with external collaborators remains pending.
...
`--max-lines-sql` deliberately defaults to `500` to keep exploratory and test runs bounded. For **PostgreSQL dumps**, it limits the rows read from each selected table. In the default **non-streaming** OMOP conversion, it also limits the number of individuals emitted, including when the input is CSV/TSV or in-memory API data. CSV/TSV rows processed with `--stream` are not capped by this option.

For a complete conversion, set the value high enough for both the largest required SQL table and the expected number of individuals.
