...
Dataset-JSON follows a different policy. The current SDTM profile does not run
mapping-file ontology searches; when a target field requires a term, it emits
a source-derived `CDISC:` identifier that records the SDTM field and value.
See [Dataset-JSON to BFF](datasetjson2bff) for the exact mapping and terminology
boundary.
