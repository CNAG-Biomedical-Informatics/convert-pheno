...
If you only need commands, use [Choose a Conversion](conversion-recipes). Use this page when you want to understand why mappings are structured the way they are.
:::
...
Source handling is route-specific rather than one global algorithm. A mapped value may populate a first-class target field and retain its source provenance, while unsupported or unconfigured values may be omitted. If required target data cannot be constructed, the route may apply a documented default, omit the affected item, or stop the conversion.
...
Converting between data standards can lose information because their schemas do not represent every concept in the same way. Where a route supports it, `Convert-Pheno` reduces that loss by retaining source context as `additionalProperties` allowed by the Beacon v2 Models [schema](https://docs.genomebeacons.org/schemas-md/individuals_defaultSchema/). This can keep original variables available for auditing and later queries, including in non-relational databases such as MongoDB. Preservation is not universal, so users should review the relevant mapping table and validate the generated output.
