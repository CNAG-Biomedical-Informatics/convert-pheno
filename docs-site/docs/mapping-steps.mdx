...
For **most routes**, `Convert-Pheno` first maps the input data to [BFF](bff), the Beacon v2 Models-based format that acts as the **internal target** or **center model**. From there, the data can remain as `BFF` or continue to other outputs such as `PXF` or `OMOP CDM`.
