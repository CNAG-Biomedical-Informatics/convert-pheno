...
[cBioPortal clinical study](../cbioportal) | Supported; experimental profile | Supported; experimental profile | Not available
...
[OpenClinica ODM](../openclinica) | Supported; experimental profile | Supported; experimental profile | Not available
...
The HTTP(s) API is intended primarily for self-contained payloads. Mapping-file-based conversions such as CSV, REDCap, and CDISC-ODM depend on additional files and are therefore better handled through the CLI. cBioPortal accepts a directory or ZIP package, and Dataset-JSON accepts multiple domain documents, so their file-oriented routes are also not exposed through the HTTP(s) API.
