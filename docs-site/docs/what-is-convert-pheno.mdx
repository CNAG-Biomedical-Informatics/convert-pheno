...
For most users, the [command-line interface](use-as-a-command-line-interface) is the **right entry point**. It is the most direct way to run conversions on files and the interface that the rest of the project is built around.
...
See [Use as a command-line interface](use-as-a-command-line-interface) for the CLI entry points and [Usage](usage) for more examples.

## Other ways to use it

If you need tighter integration:

- [As a module](use-as-a-module): call `Convert::Pheno` from Perl code
- [As an API](use-as-an-api): run the lightweight HTTP API
- [Web App UI](https://cnag-biomedical-informatics.github.io/convert-pheno-ui/): inspect and try conversions interactively

## What to read next

- [Supported Formats](supported-formats) if you want to know what can be converted
- [Download & Installation](download-and-installation) if you want to install it
- [Use as a command-line interface](use-as-a-command-line-interface) if you want to run it now
